...
class PocketMineYml_Chunksendingb8404e9817294c8ce93ee4f784995316 implements JSONModelInterface
{
    ...
    public function getRawModelDataInput(): array
    {
        return $this->_rawModelDataInput;
                ...
                : int
            {
                 ...
                 *
                 * @param int $perTick Maximum number of chunks to send to each player per tick. Higher values cause faster terrain loading, but potentially more lag.
                 *
                 * @throws ValidationException
                 *
                 * @return self
  ...
  1 => 1,
                ...
                : int
            {
                ...
                /**
                 * Set the value of spawn-radius.
                 *
                 * @param int $spawnRadius Radius of chunks to send during player join before spawning.
                 *
                 * @throws ValidationException
                 *
                 * @return self
                 */
                public function setSpawnRadius(
                    int $spawnRadius
                ): self {
                    if ($this->spawnRadius === $spawnRadius) {
                        return $this;
                    }

                    $value = $modelData['spawn-radius'] = $spawnRadius;

                    

                    

                    $value = $this->validateSpawnRadius($value, $modelData);

                    

                    $this->spawnRadius = $value;
                    $this->_rawModelDataInput['spawn-radius'] = $spawnRadius;
